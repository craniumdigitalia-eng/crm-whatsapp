...
| Auditoria robustez (plataforma inteira) | 2026-07-09 | ⚠️ CONCERNS | **P0×3**: (1) nenhuma chamada externa tem timeout (OpenAI/Evolution/Google/SMTP/Meta) → risco de estourar maxDuration 60; (2) RLS negócio (005) ainda não aplicado; (3) ingress públicos sem rate limit/cap payload (DoS de custo no webhook→OpenAI). **P1×6**: webhook processa batch síncrono serial; getAgentEnabled fail-open; kill-switch requireUser sem auditoria; telefone lead manual não E.164; campanha email serial sem maxDuration; config.required só warn. **P2×6** (segredos plaintext em integrations_config, sem tokens/custo, etc). Sólido: idempotência, claim atômico, ingress fail-closed, HMAC, RBAC, service_role server-only. Ver auditoria-robustez-2026-07-09.md | tessera (crm-qa) |
| Hardening P0-1 + P0-3 (robustez) | 2026-07-09 | ✅ IMPLEMENTADO | **P0-1 RESOLVIDO**: OpenAI `timeout:25_000 maxRetries:0`; Evolution todos os fetches com `AbortSignal.timeout(10_000)`; Calendar API (token + create/list/patch/delete) `10_000ms`; Meta Graph API `10_000ms`; nodemailer `connectionTimeout/greetingTimeout/socketTimeout` `10_000ms` cada. Build PASS. **P0-3 RESOLVIDO**: cap de payload (webhook/leadgen 64KB; site-lead 8KB; unsub POST 4KB) antes do `req.json()`; rate limit via `src/lib/rate-limit.ts` + `public.rate_limits` no Supabase (UPSERT atomico, janela fixa, fail-open em erro de banco); limites: webhook 60/min, leadgen 30/min, site-lead 10/min, email-open 120/min, email-click 60/min, unsub 20/min. Migration `016-rate-limit.sql` criada (NAO aplicada — lead aplica). P0-2 (RLS 005) permanece pendente (banco, fora do escopo Wraith). | wraith (crm-delta) |
